...
Black-Scholes-Merton PDE에서 가장 중요한 점은 주식의 기대수익률인 $\mu$가 사라진다는 것이다.

처음 주식 가격 과정은 다음과 같았다.

$$
dS_t = S_t(\mu dt + \sigma dB_t)
$$

여기에는 분명히 $\mu$가 들어 있었다.
...
이고, 여기에는 $\mu$가 없다.
 ...
 옵션 가격을 결정할 때 주식의 실제 기대수익률 $\mu$는 직접적으로 필요하지 않다는 점을 알 수 있다.

이 점은 직관적으로 꽤 놀랍다. 왜냐하면 $\mu$는 투자자들이 위험을 얼마나 싫어하는지, 즉 위험회피 성향을 반영하는 값이기 때문이다. 위험을 더 싫어하는 투자자라면 위험자산을 보유하기 위해 더 높은 기대수익률을 요구할 것이고, 그 결과 $\mu$는 달라질 수 있다.

하지만 Black-Scholes-Merton 모형에서는 이런 위험선호가 옵션 가격 공식에 직접적으로 $\mu$의 형태로 들어가지 않는다.

대신 시장에서 관찰 가능한 현재 주가 $S_0$에 이미 반영되어 있다고 본다.
